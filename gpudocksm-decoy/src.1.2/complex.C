...
// All move functions have been taken out of the calculateEnergy funtion, and placed into separate functions.
// During the MC steps, the changes to the coordinates are accumultated for all types of permutations of the ligand, the the net
		...
		if ((*p1_i).size() > 53) {
			if ((*p1_i).substr(0, 6) == "ATOM  ") {
				...
				if (atom1 == " CA ") {
					...
					switch (residue1) {
					case 0:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 2, residue1, 2));
						break;
					case 1:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 21, residue1, 2));
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 22, residue1, 3));
						break;
					case 2:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 3, residue1, 2));
						break;
					case 3:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 18, residue1, 2));
						break;
					case 4:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 14, residue1, 2));
						break;
					case 5:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 11, residue1, 2));
						break;
					case 6:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 5, residue1, 2));
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 6, residue1, 3));
						break;
					case 7:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 9, residue1, 2));
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 10, residue1, 3));
						break;
					case 8:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 26, residue1, 2));
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 27, residue1, 3));
						break;
					case 9:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 24, residue1, 2));
						break;
					case 10:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 15, residue1, 2));
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 16, residue1, 3));
						break;
					case 11:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 4, residue1, 2));
						break;
					case 13:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 7, residue1, 2));
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 8, residue1, 3));
						break;
					case 14:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 12, residue1, 2));
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 13, residue1, 3));
						break;
					case 15:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 19, residue1, 2));
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 20, residue1, 3));
						break;
					case 16:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 23, residue1, 2));
						break;
					case 17:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 17, residue1, 2));
						break;
					case 18:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 28, residue1, 2));
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 29, residue1, 3));
						break;
					case 19:
						_protein_xyz.push_back(CoordsProtein(_pnr, _pnp++, 25, residue1, 2));
						break;
					}
				...
				if ((*p1_i).substr(0, 6) == "ATOM  ") {
					...
					if (residue4 == residue3) {
						if (point1 == 0 && atom2 == " CA ") {
							...
							tn1 += 1.0;
						} else if (point1 == 2 || point1 == 3 || point1 == 4 || point1 == 11 || point1 == 14
								   || point1 == 17 || point1 == 18 || point1 == 23 || point1 == 24 || point1 == 25) {
						...
						} else if (point1 == 5 || point1 == 7 || point1 == 9 || point1 == 12 || point1 == 15
								   || point1 == 19 || point1 == 21 || point1 == 26 || point1 == 28) {
							if (atom2 != " N  " && atom2 != " CA " && atom2 != " C  " && atom2 != " O  "
								&& atom2 != " CB " && atom2 != " CG ") {
								tx1 += tx2;
								ty1 += ty2;
						...
						} else if (point1 == 6 || point1 == 8 || point1 == 10 || point1 == 13 || point1 == 16
								   || point1 == 20 || point1 == 22 || point1 == 27 || point1 == 29) {
					...
					if (point1 == 1 && residue5 > 0 && residue5 < _pnr - 1) {
						...
						if ((residue4 == residue3 && atom2 == " N  ") || (residue4 == residue6 && atom2 == " C  ")
	...
	while (getline(h1_file, line1)) {
		if (line1.length() > 3) {
			if (line1.substr(0, 3) == "KDE") {
				...
				while (dat3)
					dat3 >> dat1[dat2++];
				if (getLigCode(dat1[1]) != BADKDE) {
					//CHANGE SO THAT LINE IS SKIPPED WHEN UNKNOWN ATOM FOUND   
					_kde_xyz.push_back(CoordsKDE
									   (_pnk++, getLigCode(dat1[1]), atof(dat1[2].c_str()), atof(dat1[3].c_str()),
										atof(dat1[4].c_str())));
				...
				if (dat1[1] == _ligand_id) {
					...
					for (int ia = 0; ia < tmp_mcs.total; ia++) {
						tmp_mcs.xcoord.insert(pair < int,
											  double >(atoi(dat1[ia * 4 + 4].c_str()), atof(dat1[ia * 4 + 5].c_str())));
						tmp_mcs.ycoord.insert(pair < int,
											  double >(atoi(dat1[ia * 4 + 4].c_str()), atof(dat1[ia * 4 + 6].c_str())));
						tmp_mcs.zcoord.insert(pair < int,
											  double >(atoi(dat1[ia * 4 + 4].c_str()), atof(dat1[ia * 4 + 7].c_str())));
					}
...
bool Complex::loadLigand(std::string llib1[], int llib2, std::string llib3)
{
	...
	for (int i1 = 4 + _lna + _lnb; i1 < llib2 - 1; i1++) {
		if (llib1[i1].find(llib3) != string::npos)
			...
			for (i2 = llib1[i1 + 1].begin(), i3 = 0; i2 < llib1[i1 + 1].end(); i2++, i3++)
				switch (*i2) {
				case '0':
					_ligand_fpt_smiles[MAXFP1 - 1 - i3] = 0;
					break;
				case '1':
					_ligand_fpt_smiles[MAXFP1 - 1 - i3] = 1;
					break;
				}
		}
			...
			for (i2 = llib1[i1 + 1].begin(), i3 = 0; i2 < llib1[i1 + 1].end(); i2++, i3++)
				switch (*i2) {
				case '0':
					_ligand_fpt_maccs[MAXFP2 - 1 - i3] = 0;
					break;
				case '1':
					_ligand_fpt_maccs[MAXFP2 - 1 - i3] = 1;
					break;
				}
		}
	...
	for (int i1 = 4; i1 < _lna + 4; i1++) {
		...
		_ligand_trial.push_back(CoordsLigandTrial
								(i1 - 4, llib1[i1].substr(31, 24).c_str(), tmp3[i1 - 4], tmp2[i1 - 4]));
 ...
 */
	...
	for (i4 = _ligand_xyz.begin(), il4 = _ligand_best.begin(), it7 = _ligand_trial.begin();
		 i4 < _ligand_xyz.end() || il4 < _ligand_best.end() || it7 < _ligand_trial.end(); i4++, il4++, it7++) {
		(*i4).setCoords(tmp8[(*i4).getAtomNumber()][0], tmp8[(*i4).getAtomNumber()][1], tmp8[(*i4).getAtomNumber()][2],
						_lens_total);
		(*il4).setCoords(tmp8[(*il4).getAtomNumber()][0], tmp8[(*il4).getAtomNumber()][1],
						 tmp8[(*il4).getAtomNumber()][2], _lens_total);
		(*it7).setCoords(tmp8[(*it7).getAtomNumber()][0], tmp8[(*it7).getAtomNumber()][1],
						 tmp8[(*it7).getAtomNumber()][2], _lens_total);
	}
	...
	for (int i6 = 0; i6 < tmp7; i6++) {
		...
		if (rms2 > 0.1) {
			for (i4 = _ligand_xyz.begin(); i4 < _ligand_xyz.end(); i4++)
				(*i4).setCoords(mob_xyz[(*i4).getAtomNumber()][0], mob_xyz[(*i4).getAtomNumber()][1],
								mob_xyz[(*i4).getAtomNumber()][2], _lens_total);
...
void Complex::ligPerturb(int ens1)
{
	...
	for (ens2 = _ligand_xyz.begin(), ens3 = 0; ens2 < _ligand_xyz.end(); ens2++, ens3++) {
		...
		for (int ens4 = 0; ens4 < 3; ens4++)
			ref_xyz[ens3][ens4] = (*ens2).getCoords(ens4 + 1, _lens_current);
	}

	vector < CoordsLigand >::iterator ens5;
	for (ens5 = _ligand_xyz.begin(), ens3 = 0; ens5 < _ligand_xyz.end(); ens5++, ens3++) {
		for (int ens4 = 0; ens4 < 3; ens4++) {
			mob_xyz[ens3][ens4] = _ref_coords[ens3][ens4][ens1];
		}
	}
	...
	for (atom = 0, ens6 = _ligand_xyz.begin(); ens6 < _ligand_xyz.end(); atom++, ens6++) {
		_perturb_delta[atom][0] =
			(t[0] + u[0][0] * mob_xyz[atom][0] + u[1][0] * mob_xyz[atom][1] + u[2][0] * mob_xyz[atom][2] -
			 (*ens6).getCoords(1, ens1));
		_perturb_delta[atom][1] =
			(t[1] + u[0][1] * mob_xyz[atom][0] + u[1][1] * mob_xyz[atom][1] + u[2][1] * mob_xyz[atom][2] -
			 (*ens6).getCoords(2, ens1));
		_perturb_delta[atom][2] =
			(t[0] + u[0][2] * mob_xyz[atom][0] + u[1][2] * mob_xyz[atom][1] + u[2][2] * mob_xyz[atom][2] -
			 (*ens6).getCoords(3, ens1));
...
///CALCULAT THE CHANGE IN COORDS DUE TO ROTATION////////
	for (int a = 0; a < 3; a++) {
		cxyz[a] = 0.0;
		for (int j = 0; j < MAXLIG; j++)
			rxyz[j][a] = 0.0;
	...
	for (il6 = _ligand_xyz.begin(), il2 = _ligand_trial.begin(), atom = 0; il6 < _ligand_xyz.end();
		 il6++, il2++, atom++) {

		x = (*il6).getCoords(1, _lens_current);
		y = (*il6).getCoords(2, _lens_current);
		z = (*il6).getCoords(3, _lens_current);
///ADD CHANGE FROM PERTURBATION  
...
///This adds the change in coordinates due to translation and rotation//
		...
		x += _ligand_tra[0] + (oxyz[0] + cxyz[0] - x);
		y += _ligand_tra[1] + (oxyz[1] + cxyz[1] - y);
		z += _ligand_tra[2] + (oxyz[2] + cxyz[2] - z);
	...
	_ligand_center[0] = x_cent;
	_ligand_center[1] = y_cent;
	_ligand_center[2] = z_cent;
...
void Complex::calculateEnergy(void)
{
 ...
 for ( il7 = _ligand_trial.begin(), il1 = _ligand_xyz.begin(); il7 < _ligand_trial.end(); il7++, il1++ )
 {
 ...
 //////
  ...
  for ( ip1 = _protein_xyz.begin(); ip1 < _protein_xyz.end(); ip1++ )
  {
   ...
   /* contact potential */
  //Two choices for contact potential calculation// 
...
vector<CoordsLigandTrial>::iterator il9;

 for ( ip4 = _protein_xyz.begin(); ip4 < _protein_xyz.end(); ip4++ )
  for ( il7 = _ligand_trial.begin(), atom1=0; il7 < _ligand_trial.end(); il7++, atom1++ )
  {
   double rr1 = sqrt( gsl_sf_pow_int( _ref_coords[atom1][0][_lens_current] - (*ip4).getCoords(1, _pens_current) + _pocket_center[0], 2 ) +
   ...
   double rr2 = sqrt( gsl_sf_pow_int( (*il7).getCoords(1, _lens_current) - (*ip4).getCoords(1, _pens_current) + _pocket_center[0], 2 ) +
                      gsl_sf_pow_int( (*il7).getCoords(2, _lens_current) - (*ip4).getCoords(2, _pens_current) + _pocket_center[1], 2 ) +
                      gsl_sf_pow_int( (*il7).getCoords(3, _lens_current) - (*ip4).getCoords(3, _pens_current) + _pocket_center[2], 2 )   );

   double rr3 = _complex_pmf[(*ip4).getPointType()][(*il7).getAtomType()][0];

   if ( rr1 <= rr3 && rr2 <= rr3 )
    tp1 += 1.0;
   else if ( rr1 <= rr3 && rr2 > rr3 )
    fn1 += 1.0;
   else if ( rr1 > rr3 && rr2 <= rr3 )
    fp1 += 1.0;
   else
 ...
 // only _evdw and _edst considered in generating the decoys
 _etot = _evdw + _edst;
...
//===================================================================================   restoreCoords
//Set Trial Coordinates back to previous coordinates

void Complex::restoreCoords(int nbkp)	// what is nbkp here? unused ! ???
{
	vector < CoordsLigand >::iterator il1;
	vector < CoordsLigandTrial >::iterator il2;

	for (il1 = _ligand_xyz.begin(), il2 = _ligand_trial.begin(); il1 < _ligand_xyz.end(); il1++, il2++) {
		(*il2).setCoords((*il1).getCoords(1, _lens_current), (*il1).getCoords(2, _lens_current),
						 (*il1).getCoords(3, _lens_current), _lens_current);
...
// ==================================================================================   printEnerInfo
void Complex::printEnerInfo(ofstream & eout)
{
	vector < CoordsLigand >::iterator il1;
	eout << _cmcc << " ";
	for (int i = 1; i < 11; i++)
		eout << getEnergy(i) << " ";	    // why 2~9 energy terms ???
	eout << endl;
}

// ==================================================================================   printLigandInfo
void Complex::printLigandInfo(ofstream & fout, ofstream & Coefout, ofstream & eout)
{
	vector < CoordsLigand >::iterator il1;
	Coefout << _cmcc << endl;
	//fout<<_cmcc<<endl;
...
void initOutStream(OutStream * outfiles, std::string id)
{
...
#if WRITE
	std::string lig_id = id;
	int len = lig_id.length();

	cout << "preparing output files ..." << endl << endl;

	string all_MCcoef_file = lig_id.substr(0, len) += "-A-MCcoefs.txt";
	string all_Decoy_file = lig_id.substr(0, len) += "-A-Decoys.txt";
	string all_Enr_file = lig_id.substr(0, len) += "-A-DecoyEner.txt";

	outfiles->all_decoys.open(all_Decoy_file.c_str(), ios::trunc);
	outfiles->all_MCcoef.open(all_MCcoef_file.c_str(), ios::trunc);
	outfiles->all_decoyenergies.open(all_Enr_file.c_str(), ios::trunc);

	cout << all_MCcoef_file << "... done " << endl;
	cout << all_Decoy_file << "... done " << endl;
	cout << all_Enr_file << "... done " << endl << endl;

	// string L_MCcoef_file = lig_id.substr(0, len) += "-L-MCcoefs.txt";
	// string L_Decoy_file = lig_id.substr(0, len) += "-L-Decoys.txt";
	// string L_Enr_file = lig_id.substr(0, len) += "-L-DecoyEner.txt";

	// outfiles->L_decoys.open(L_Decoy_file.c_str(), ios::trunc);
	// outfiles->L_MCcoef.open(L_MCcoef_file.c_str(), ios::trunc);
	// outfiles->L_decoyenergies.open(L_Enr_file.c_str(), ios::trunc);
	// cout << L_MCcoef_file << "... done " << endl;
	// cout << L_Decoy_file << "... done " << endl;
	// cout << L_Enr_file << "... done " << endl << endl;

	// string H_MCcoef_file = lig_id.substr(0, len) += "-H-MCcoefs.txt";
	// string H_Decoy_file = lig_id.substr(0, len) += "-H-Decoys.txt";
	// string H_Enr_file = lig_id.substr(0, len) += "-H-DecoyEner.txt";

	// outfiles->H_decoys.open(H_Decoy_file.c_str(), ios::trunc);
	// outfiles->H_MCcoef.open(H_MCcoef_file.c_str(), ios::trunc);
	// outfiles->H_decoyenergies.open(H_Enr_file.c_str(), ios::trunc);
	// cout << H_MCcoef_file << "... done " << endl;
	// cout << H_Decoy_file << "... done " << endl;
	// cout << H_Enr_file << "... done " << endl << endl;

	// write the titles
	outfiles->all_decoyenergies << "cmcc " << "_etot " << "_evdw " << "_eele " << "_epmf " << "_ehpc " << "_ehdb " << "_edst " << "_epsp "<< "_ekde " << "_elhm" << endl;
	// outfiles->L_decoyenergies << "_evdw " << "_eele " << "_epmf " << "_ehpc " << "_ehdb " << "_edst " << "_epsp "<< "_ekde " << "_elhm" << endl;
	// outfiles->H_decoyenergies << "_evdw " << "_eele " << "_epmf " << "_ehpc " << "_ehdb " << "_edst " << "_epsp "<< "_ekde " << "_elhm" << endl;
#endif
}
...
{
	outfiles->all_decoys.close();
	outfiles->all_MCcoef.close();
	outfiles->all_decoyenergies.close();

	// outfiles->L_decoys.close();
	// outfiles->L_MCcoef.close();
	// outfiles->L_decoyenergies.close();

	// outfiles->H_decoys.close();
	// outfiles->H_MCcoef.close();
	// outfiles->H_decoyenergies.close();
}
