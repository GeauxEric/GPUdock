				...
				if ((*p1_i).substr(0, 6) == "ATOM  ") {
					...
					if (residue4 == residue3) {
						if (point1 == 0 && atom2 == " CA ") {
							...
							tn1 += 1.0;
						} else if (point1 == 2 || point1 == 3 || point1 == 4 || point1 == 11
							   || point1 == 14 || point1 == 17 || point1 == 18
						...
						} else if (point1 == 5 || point1 == 7 || point1 == 9 || point1 == 12
							   || point1 == 15 || point1 == 19 || point1 == 21
							   || point1 == 26 || point1 == 28) {
							if (atom2 != " N  " && atom2 != " CA " && atom2 != " C  "
						...
						} else if (point1 == 6 || point1 == 8 || point1 == 10 || point1 == 13
							   || point1 == 16 || point1 == 20 || point1 == 22
							   || point1 == 27 || point1 == 29) {
...
bool Complex::loadLigand(std::string llib1[], int llib2, std::string llib3)
{
	...
	for (int i1 = 4; i1 < _lna + 4; i1++) {
		// cout<<tmp3[i1-4]<<endl;
		_ligand_xyz.
		    push_back(CoordsLigand(i1 - 4, llib1[i1].substr(31, 24).c_str(), tmp3[i1 - 4], tmp2[i1 - 4]));
...
// vector<CoordsLigandTrial>::iterator it7;
	for (i4 = _ligand_xyz.begin() /*, il4 = _ligand_best.begin(), it7 = _ligand_trial.begin() */ ;
	     i4 < _ligand_xyz.end() /*|| il4 < _ligand_best.end()|| it7 < _ligand_trial.end() */ ;
	     i4++ /*, il4++, it7++ */ ) {
		(*i4).setCoords(tmp8[(*i4).getAtomNumber()][0], tmp8[(*i4).getAtomNumber()][1],
				tmp8[(*i4).getAtomNumber()][2], _lens_total);
...
void Complex::createConfusionMatrix(void)
{
	...
	for (ip4 = _protein_xyz.begin(); ip4 < _protein_xyz.end(); ip4++, col++) {
		for (il7 = _ligand_xyz.begin(); il7 < _ligand_xyz.end(); il7++, row++) {
			double rr1 =
			    sqrt(pow
				 ((*il7).getCoords(1, _lens_current) - (*ip4).getCoords(1, _pens_current) +
				  _pocket_center[0], 2) + pow((*il7).getCoords(2, _lens_current) - (*ip4).getCoords(2,
														    _pens_current)
							      + _pocket_center[1], 2) + pow((*il7).getCoords(3,
													     _lens_current)
											    - (*ip4).getCoords(3,
													       _pens_current)
											    + _pocket_center[2], 2));
...
void Complex::clearConfusionMatrix(void)
{
...
// ==================================================================================   calculate Mattew's correlation coefficient
void Complex::calculateMccMtrx( void ){
	_cmcc = 0.0;
	...
	for (ip4 = _protein_xyz.begin(), ppoint = 0; ip4 < _protein_xyz.end(); ip4++, ppoint++)
		for (il7 = _ligand_xyz.begin(), latom = 0; il7 < _ligand_xyz.end(); il7++, latom++) {
			int rr1 = _confusion_matrix[latom][ppoint];	
			double rr2 =
			    sqrt(pow
				 ((*il7).getCoords(1, _lens_current) - (*ip4).getCoords(1, _pens_current) +
				  _pocket_center[0], 2) + pow((*il7).getCoords(2, _lens_current) - (*ip4).getCoords(2,
			...
			if (rr1 == 1 && rr2 <= rr3)
				tp1 += 1.0;
			else if (rr1 == 1 && rr2 > rr3)
				fn1 += 1.0;
			else if (rr1 == 0 && rr2 <= rr3)
				fp1 += 1.0;
			else
				tn1 += 1.0;
		}
...
// ==================================================================================   return Mattew's correlation coefficient
double Complex::getAmcc ( void ){
	return _cmcc;
}
...
*/
	...
	for (ip4 = _protein_xyz.begin(), ppoint = 0; ip4 < _protein_xyz.end(); ip4++, ppoint++)
		for (il7 = _ligand_xyz.begin(), latom = 0; il7 < _ligand_xyz.end(); il7++, latom++) {
			//double rr1=5.0;
			int rr1 = _confusion_matrix[latom][ppoint];	// sqrt( gsl_sf_pow_int( _ref_coords[atom1][0][_lens_current] - (*ip4).getCoords(1, _pens_current) + _pocket_center[0], 2 ) +
