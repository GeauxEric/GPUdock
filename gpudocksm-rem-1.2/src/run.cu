     ...
     const ComplexSize complexsize)
{
  ...
  const size_t ligmovematrix_sz = sizeof (LigMoveMatrix) * n_rep;
  ...
  float *etotal_d[NGPU];
  LigMoveMatrix *ligmovematrix_d[NGPU];
  ...
  for (int i = 0; i < NGPU; ++i) {
    ...
    cudaMalloc ((void **) &ligmovematrix_d[i], ligmovematrix_sz);
    ...
    CUDAMEMCPYTOSYMBOL (ligmovematrix_dc, &ligmovematrix_d[i], LigMoveMatrix *);
