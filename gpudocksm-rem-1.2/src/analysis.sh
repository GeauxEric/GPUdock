...
hd_paths=$(ls ../testing/output_*/*.h5)
rep=$1   # replica number you check
temp=$(cat report | grep "total temperatures"|awk '{print $3}')

printf "start analyszing # %d replica\n" "$rep" 
...
for hd_path in $hd_paths; do
    ...
    ./analysis -num_temp $temp -rep $rep -nl 4500 -l 1 -p 2 -e $hd_path > $csv_path
done
