...
*** TODO figure out the rotation scale
    currently ratational scale is 2-pi, meaning that the ligand could be rotated
    to any degrees in one perturbation
...
**** TODO function to print the replica number and the corresponding temperature
     need to find out the replica with the lowest temperature after all running done

**** TODO caculate the exchange acceptance ratio
