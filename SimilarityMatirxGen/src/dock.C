...
main (int argc, char **argv)
{
...
#if IS_CALCU_TRACE == 1
  float trace[7];
  loadTrace(&inputfiles->trace_file, trace);

  int lig_conf = (int)trace[0];
  Ligand *mylig = &lig[lig_conf];

  float movematrix_new[6];
  for (int i=0; i < 6; i++) {
    movematrix_new[i] = trace[i+1];
  }
  
  Pl
  aceLigand(mylig, movematrix_new);
  string ofn = "LigCoord.txt";
  
  PrintLigCoord2File(mylig, ofn);
#endif
  ...
  // declare reference matrix and similarity matrix
  int lna = lig->lna;
  int pnp = prt->pnp;
  int tot_traces = trace_matrix.size();
  float *similarity_matrix = new float [tot_traces * tot_traces];
  int *ref_matrix = new int[lna * pnp];
  
  // run
  CalculateSimilarityMatrix(trace_matrix, similarity_matrix, ref_matrix, lig, prt, enepara);

  // save to file
  string simi_path = inputfiles->near_native_trace_file.simi_path;
  SaveSimilarityMatrix(similarity_matrix, tot_traces, simi_path);
