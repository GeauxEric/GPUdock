...
struct NearNativeTraceFile
{
  std::string trace_path;  // near native traces file path
  std::string simi_path; // output file path
};
...
struct InputFiles
{
  ...
  NearNativeTraceFile near_native_trace_file;
};
...
#if COMPUTE_DEVICE == USE_GPU
#include "dock_gpu.h"
#elif COMPUTE_DEVICE == USE_CPU || COMPUTE_DEVICE == USE_MIC
#include "dock_cpu.h"
#endif
