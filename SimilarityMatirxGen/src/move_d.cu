...
// move the ligand away initialy
__device__ void
MoveAway_d (const int bidx, Ligand * __restrict__ mylig)
{
  __shared__ float movematrix_new[6]; // translation x y z, rotation x y z
  __shared__ float rot[3][3]; // rot_z rot_y rot_x
...
__device__ void
Move_d (const int bidx, Ligand * __restrict__ mylig)
{
  __shared__ float movematrix_new[6]; // translation x y z, rotation x y z
