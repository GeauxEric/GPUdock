...
void
CalculateSimilarityMatrix(vector < vector < float > > trace_matrix, 
			  float * similarity_matrix, int * ref_matrix, 
			  Ligand * lig, Protein * prt, EnePara * enepara)
{
  for (vector < vector < float > > :: iterator it0 = trace_matrix.begin(); 
       it0 != trace_matrix.end(); it0++) {

    int tot_traces = trace_matrix.size();
    int idx0 = it0 - trace_matrix.begin();

    vector < float > conf0 = *it0;
    float my_conf0[8];
    for (int i = 0; i < 8; i++)
      my_conf0[i] = conf0.at(i);
    
    int prt_conf0 = my_conf0[0];
    int lig_conf0 = my_conf0[1];
    float *mv_vec0 = &my_conf0[2];
    Ligand *my_lig0 = &lig[lig_conf0];
    Protein *my_prt0 = &prt[prt_conf0];
    
    // move the ligand
    PlaceLigand(my_lig0, mv_vec0);

    // create contact matrix for conf0
    InitConfMatrix(ref_matrix, my_lig0, my_prt0, enepara);
      
    for (vector < vector < float > > :: iterator it1 = trace_matrix.begin(); 
	 it1 != trace_matrix.end(); it1++) {
      int idx1 = it1 - trace_matrix.begin();

      vector < float > conf1 = *it1;
      float my_conf1[8];
      for (int i = 0; i < 8; i++)
	my_conf1[i] = conf1.at(i);

      int prt_conf1 = my_conf1[0];
      int lig_conf1 = my_conf1[1];
      float *mv_vec1 = &my_conf1[2];
      Ligand *my_lig1 = &lig[lig_conf1];
      Protein *my_prt1 = &prt[prt_conf1];
      
      // move the ligand
      PlaceLigand(my_lig1, mv_vec1);

      // calculate contact mode score
      float cms = CalculateContactModeScore(ref_matrix, my_lig1, my_prt1, enepara);
      similarity_matrix[idx0 * tot_traces + idx1] = cms;
    }
  }
}

void
InitConfMatrix (int * ref_matrix, Ligand * mylig, Protein * myprt, EnePara * enepara)
{
  int lna = mylig->lna;
  int pnp = myprt->pnp;
...
void
PrintInitConfMatrix (int * ref_matrix, Ligand * mylig, Protein * myprt, EnePara * enepara)
{
  int lna = mylig->lna;
  int pnp = myprt->pnp;
  cout << "lna" << lna << endl;
...
SaveSimilarityMatrix (float * similarity_matrix, int sz, string simi_path) 
{
  ...
  of.close();
  cout << "writing near native similarity matrix to " << simi_path << endl;
}

float
CalculateContactModeScore (int * ref_matrix, Ligand * mylig, Protein * myprt, EnePara * enepara)
{
  int tp = 0;
  int fn = 0;
  int fp = 0;
  ...
  for (int l = 0; l < lna; l++) {
    ...
    for (int p = 0; p < pnp; p++) {
      ...
      const float pmf0 = enepara->pmf0[lig_t][prt_t];
      const int ref_val = ref_matrix[l * pnp + p];
      tp += (ref_val == 1 && dst <= pmf0);
      fn += (ref_val == 1 && dst > pmf0);
      fp += (ref_val == 0 && dst <= pmf0);
      tn += (ref_val == 0 && dst > pmf0);
    }
  }
...
void
SimilarityMatrixBanner ()
{
  cout << "------------------------------------------------------------" << endl
       << "                         GeauxDock                          " << endl
       << "                        version 0.1                         " << endl << endl
       << "                      Auxiliary function    " << endl
       << "                 Calculating Similarity Matrix          " << endl
       << "------------------------------------------------------------" << endl << endl;
}
		...
		InputFiles * inputfiles)
{
  ...
  for (int i = 0; i < argc; i++) {
    ...
    // single conformation trace file
    if (!strcmp (argv[i], "-tr") && i < argc) {
      inputfiles->trace_file.path = argv[i + 1];
    }

    // near native conformations trace file
    if (!strcmp (argv[i], "-ntr") && i < argc) {
      inputfiles->near_native_trace_file.trace_path = argv[i + 1];
    }

    // near native conformations similarity matrix file
    if (!strcmp (argv[i], "-simi") && i < argc) {
      inputfiles->near_native_trace_file.simi_path = argv[i + 1];
    }
...
OptimizeEnepara (const EnePara0 * enepara0, EnePara * enepara)
{
  ...
  for (int i = 0; i < MAXTP2; ++i) {	// lig
    for (int j = 0; j < MAXTP1; ++j) {	// prt
      ...
      //enepara->hdb2[i][j] = -1.0f / (enepara0->hdb[j][i][1] * sqrt_2_pi); // using hdb2 slightly improves the performance
    }
  }
