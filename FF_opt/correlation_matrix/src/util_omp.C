...
ParseArguments (int argc, char **argv, McPara * mcpara, ExchgPara * exchgpara,
		InputFiles * inputfiles, OutputFiles * outputfile)
{
...
*/
  ...
  for (int i = 0; i < argc; i++) {
    ...
    if (!strcmp (argv[i], "-tr") && i < argc) {
      inputfiles->track_file.path = argv[i + 1];
    }
    if (!strcmp (argv[i], "-o") && i < argc) {
      outputfile->corr_mat_path = argv[i + 1];
    }
    if (!strcmp (argv[i], "-total_row") && i < argc) {
      inputfiles->track_file.total_rows = atoi(argv[i + 1]);
    }
...
void
InitRefMat (int * contact_matx_ref, const Ligand * mylig, const Protein * myprt, const EnePara * enepara)
{
  const int lna = mylig->lna;
  const int pnp = myprt->pnp;
...
float
CalcCorrMat (const int * contact_matx_ref, const Ligand * mylig, const Protein * myprt, const EnePara * enepara)
{
  const int lna = mylig->lna;
  const int pnp = myprt->pnp;
  ...
  return (dividend != 0) ? ((tp * tn - fp * fn) / dividend) : CMCC_INVALID_VAL;
}

// void
// PrepareLigPrt (Ligand * mylig, Protein * prt, float * track_addr){
//   float move_vect[6];
//   memcpy (move_vect, track_addr, sizeof(float) * 6);
//   const int lig_conf = (int) track_addr[6];
//   const int prt_conf = (int) track_addr[7];
  
//   mylig = &lig[lig_conf];
//   myprt = &prt[prt_conf];
  
//   MoveLig (mylig, move_vect);
// }
