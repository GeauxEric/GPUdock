  ...
  for (int word = 0; word < NWORD; word++) {
	...
	for (int z_offset = 0; z_offset < SZ; z_offset += BDz0) {
	  ...
	  for (int b = 0; b < NBETA_PER_WORD; b++) {
	    ...
	    //energy = energy * 2 - 6;
	    //spin = spin * 2 - 1;
	    //PROB_DATATYPE val = __expf (2 * temp_prob_shared[b][0] * (energy - H * spin));

            //*/


	    PROB_DATATYPE val = (energy << 1) + spin;
	    //PROB_DATATYPE val = temp_prob_shared[b][(energy << 1) + spin];
	    //PROB_DATATYPE val = tex1D(mytexture, (energy << 1) + spin, b);
	    //PROB_DATATYPE val = tex2D(mytexture, (energy << 1) + spin, b);
	    //PROB_DATATYPE val = energy ^ spin;
	    //PROB_DATATYPE val = temp_prob_shared[b][0] * (energy - H * spin);
