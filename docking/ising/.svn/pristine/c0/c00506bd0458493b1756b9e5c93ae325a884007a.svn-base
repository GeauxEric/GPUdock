...
texture <PROB_DATATYPE, 2, cudaReadModeElementType> mytexture;
...
stencil (float *temp_beta_shared, int iter)
{
  ...
  /*
  // testing the y index of probability table for 2-step-referencing
  __shared__ int prob_y_idx[NBETA];
  if (bidx < NBETA)
    prob_y_idx[bidx] = bidx;
  */
	...
	for (int z_offset = 0; z_offset < SZ; z_offset += BDz0) {
	  ...
	  for (int offset = 0; offset < NBETA_PER_SEG; ++offset) {
	    ...
	    for (int seg = 0; seg < NSEG_PER_WORD; ++seg) {
	      ...
	      //int b = prob_y_idx[NBETA_PER_SEG * seg + offset];
	      int position = (seg << 2) + offset;


	      //this is wrong, should test with AMSC-1 or AMSC-3
	      //int energy = (e >> (seg << 2)) & MASK_E;
	      //energy = energy * 2 - 6;
	      //int spin = b * 2 - 1;


	      PROB_DATATYPE val = temp_prob_shared[b][(e >> (seg << 2)) & MASK_E];
	      //PROB_DATATYPE val = tex1D(mytexture, (e >> (seg << 2)) & MASK_E);
	      //PROB_DATATYPE val = tex2D(mytexture, (e >> (seg << 2)) & MASK_E, b);

	      PROB_DATATYPE myrand = curand (&seed0);	// range: [0,U_INT32_T_MAX]
	      //PROB_DATATYPE myrand = curand_uniform (&seed0);	// range: [0,U_INT32_T_MAX]
	      flip |= ((myrand < val) << position);	// myrand < val ? 1 : 0;
	      //flip |= ((myrand < b + position) << position);	// myrand < val ? 1 : 0;
	    }
	  }
