...
stencil (float *temp_beta_shared, int iter)
{
  ...
  for (int word = 0; word < NWORD; word++) {
	...
	for (int z_offset = 0; z_offset < SZ; z_offset += BDz0) {
	  ...
	  /*
	  PROB_DATATYPE myrand1 = 7;
	  PROB_DATATYPE myrand2 = 7;
	  PROB_DATATYPE myrand3 = 7;
	  PROB_DATATYPE myrand4 = 7;
	  */
	  ...
	  for (int offset = 0; offset < NBETA_PER_SEG; ++offset) {
...
#if 0
	    // curand
	    for (int seg = 0; seg < NSEG_PER_WORD; seg += 4) {
	      int b = NBETA_PER_SEG * seg + offset;
	      int position = (seg << 2) + offset;
	      int e_shift = (seg << 2);

	      PROB_DATATYPE val1 = temp_prob_shared[b][(e >> (e_shift)) & MASK_E];
	      PROB_DATATYPE val2 = temp_prob_shared[b + 3][(e >> (e_shift + 4)) & MASK_E];
	      PROB_DATATYPE val3 = temp_prob_shared[b + 6][(e >> (e_shift + 8)) & MASK_E];
	      PROB_DATATYPE val4 = temp_prob_shared[b + 9][(e >> (e_shift + 12)) & MASK_E];

	      PROB_DATATYPE myrand1 = curand (&seed0);
	      PROB_DATATYPE myrand2 = curand (&seed0);
	      PROB_DATATYPE myrand3 = curand (&seed0);
	      PROB_DATATYPE myrand4 = curand (&seed0);

	      flip =
		flip |
		((myrand1 < val1) << (position)) |
		((myrand2 < val2) << (position + 4)) |
		((myrand3 < val3) << (position + 8)) |
		((myrand4 < val4) << (position + 12));
	    }
	    ...
	    for (int seg = 0; seg < NSEG_PER_WORD; seg += 4) {
	      ...
	      gpu_lcg (&seed1);
	      PROB_DATATYPE myrand1 = (PROB_DATATYPE) seed1;
	      gpu_lcg (&seed1);
	      PROB_DATATYPE myrand2 = (PROB_DATATYPE) seed1;
	      gpu_lcg (&seed1);
	      PROB_DATATYPE myrand3 = (PROB_DATATYPE) seed1;
	      gpu_lcg (&seed1);
	      PROB_DATATYPE myrand4 = (PROB_DATATYPE) seed1;
	    ...
	    for (int seg = 0; seg < NSEG_PER_WORD; seg += 4) {
	      ...
	      /*
	      PROB_DATATYPE myrand1 = rand123_u.i.x;
	      PROB_DATATYPE myrand2 = rand123_u.i.y;
	      PROB_DATATYPE myrand3 = rand123_u.i.z;
	      PROB_DATATYPE myrand4 = rand123_u.i.w;
	      */
	      flip =
		flip |
		((myrand1 < val1) << (position)) |
	    ...
	    for (int seg = 0; seg < NSEG_PER_WORD; seg += 4) {
	      ...
	      PROB_DATATYPE myrand1 = curand_uniform (&seed0);
	      PROB_DATATYPE myrand2 = curand_uniform (&seed0);
	      PROB_DATATYPE myrand3 = curand_uniform (&seed0);
	      PROB_DATATYPE myrand4 = curand_uniform (&seed0);
	    ...
	    for (int seg = 0; seg < NSEG_PER_WORD; seg += 4) {
	      ...
	      PROB_DATATYPE myrand1 = gpu_lcg_f (&seed1);
	      PROB_DATATYPE myrand2 = gpu_lcg_f (&seed1);
	      PROB_DATATYPE myrand3 = gpu_lcg_f (&seed1);
	      PROB_DATATYPE myrand4 = gpu_lcg_f (&seed1);
	    ...
	    for (int seg = 0; seg < NSEG_PER_WORD; seg += 4) {
	      ...
	      PROB_DATATYPE myrand1 = rand123_u.i.x / U_INT32_T_MAX;
	      PROB_DATATYPE myrand2 = rand123_u.i.y / U_INT32_T_MAX;
	      PROB_DATATYPE myrand3 = rand123_u.i.z / U_INT32_T_MAX;
	      PROB_DATATYPE myrand4 = rand123_u.i.w / U_INT32_T_MAX;
