	...
	for (int z_offset = 0; z_offset < SZ; z_offset += BDz0) {
...
#if 0
	  MSC_DATATYPE c = l[z][y][x];
	  MSC_DATATYPE n0 = l[z][y][xa];
	  MSC_DATATYPE n1 = l[z][y][xb];
	  MSC_DATATYPE n2 = l[z][ya][x];
	  MSC_DATATYPE n3 = l[z][yb][x];
	  MSC_DATATYPE n4 = l[za][y][x];
	  MSC_DATATYPE n5 = l[zb][y][x];
	  l[z][y][x] = c ^ n0 ^ n1 ^ n2 ^ n3 ^ n4 ^ n5;
#endif
	  ...
	  for (int offset = 0; offset < NBETA_PER_SEG; ++offset) {
	    ...
	    for (int seg = 0; seg < 8; seg += 4) {
	      ...
	      PROB_DATATYPE myrand1 = curand (&seed0);	// range: [0,U_INT32_T_MAX]
	      PROB_DATATYPE myrand2 = curand (&seed0);	// range: [0,U_INT32_T_MAX]
	      PROB_DATATYPE myrand3 = curand (&seed0);	// range: [0,U_INT32_T_MAX]
	      PROB_DATATYPE myrand4 = curand (&seed0);	// range: [0,U_INT32_T_MAX]
	      flip =
		flip |
	    ...
	    */
	    ...
	    for (int seg = 0; seg < 8; seg += 2) {
	      ...
	      flip =
		flip |
		((myrand < val1) << (position)) |
		((myrand < val2) << (position + 4));
